...
while True:
    ...
    for particle in particles:
        params = screen, (255, 255, 255), [int(particle[0][0]), int(particle[0][1])], int(particle[2])
        pygame.draw.circle(*params)

    # Render Tiles ------------------------------------------- #
    for tile in tile_map:
        pygame.draw.rect(screen, tile_map[tile][2],
                         pygame.Rect(tile_map[tile][0] * TILE_SIZE, tile_map[tile][1] * TILE_SIZE, TILE_SIZE,
                                     TILE_SIZE))
    ...
    # Update ------------------------------------------------- #
    pygame.display.set_caption(f'{str(round(mainClock.get_fps()))}FPS {len(particles)} Particles')
